System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, Button, Component, instantiate, Node, UIOpacity, Vec3, _decorator, AS, UIDrag, UIDrop, UIView, _dec, _dec2, _dec3, _dec4, _class, _class2, _descriptor, _descriptor2, _descriptor3, _descriptor4, _crd, ccclass, property, StoryPanel;
    ...
    execute: function () {
      ...
      _export("StoryPanel", StoryPanel = (_dec = ccclass("StoryPanel"), _dec2 = property({
        ...
        constructor() {
          ...
          this._pageNumber = 0;
          this._draggables = [];
          this._panelPosition = new Vec3(0, 0, 0);
        }
        ...
        _addComponent() {
          ...
          var slot = this.panel.getComponentsInChildren(_crd && UIDrop === void 0 ? (_reportPossibleCrUseOfUIDrop({
            error: Error()
          }), UIDrop) : UIDrop);
          slot.forEach(s => {
            s.node.on((_crd && UIDrop === void 0 ? (_reportPossibleCrUseOfUIDrop({
              error: Error()
            }), UIDrop) : UIDrop).EventType.DRAG_ADDED, this._DragEnd.bind(this));
            s.node.on((_crd && UIDrop === void 0 ? (_reportPossibleCrUseOfUIDrop({
              error: Error()
            }), UIDrop) : UIDrop).EventType.DRAG_REMOVED, this._removed.bind(this));
          });

          for (var i = 0; i < this.DragContainer.children.length; i++) {
            var drag = this.DragContainer.children[i].addComponent(_crd && UIDrag === void 0 ? (_reportPossibleCrUseOfUIDrag({
              error: Error()
            }), UIDrag) : UIDrag);
            this.DragContainer.children[i].addComponent(UIOpacity); // drag.node.on(UIDrag.EventType.DRAG_DID_END, () => {
            //   this._DragEnd(drag);
            // });
        ...
        _DragEnd(drag) {
          ...
          drag.interactable = false; // if (slot.replacedNode.length != 0) {
        ...
        _removed(removed) {
          console.log(removed); // removed.node.getComponent(UIOpacity).opacity = 255;
        ...
        close() {
          ...
          return _asyncToGenerator(function* () {
            yield _this.popup.hide();

            _this.panel.setPosition(_this._panelPosition);
        ...
        _pageNavigation(pageNumber) {
          ...
          for (var _i = 0; _i < pageGrouping.length; _i++) {
            for (var _j = 0; _j < pageGrouping[_i].length; _j++) {
              if (_i == pageNumber) {
                pageGrouping[_i][_j].active = true;
              } else {
                pageGrouping[_i][_j].active = false;
              }
            }
          }
        }
...
//# sourceMappingURL=e91ad1f98c3d0180ebb9562200747dd8ba43d258.js.map