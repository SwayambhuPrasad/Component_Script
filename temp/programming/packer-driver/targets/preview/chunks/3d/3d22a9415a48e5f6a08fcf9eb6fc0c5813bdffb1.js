    ...
    execute: function () {
      ...
      _export("SoftKeypad", SoftKeypad = (_dec = ccclass("SoftKeypad"), _dec2 = property({
        ...
        awake() {
          ...
          var arrowUITransform = this.arrow.getComponent(UITransform);
        ...
        _calculatePopupSize() {
          ...
          var width = this.keysContainer.paddingLeft + this.keysContainer.paddingRight;
          var height = this.keysContainer.paddingBottom + this.keysContainer.paddingTop;

          if (this.showEmpty) {
            // Get the empty content node added.
            var emptyNode = this.keysContainer.node.children[0];
            var emptyNodeSize = emptyNode._uiProps.uiTransformComp.contentSize;
            width += emptyNodeSize.width;
            height += emptyNodeSize.height;
            return size(width, height);
          }

          var baseSize = this.keysLayout.keys.reduce((acc, key) => {
            if (key.size.width > acc.width) acc.width = key.size.width;
            if (key.size.height > acc.height) acc.height = key.size.height;
            return acc;
          }, size(0, 0));
          var keyCount = this.keysContainer.node.children.length;
          ...
          if (this.keysContainer.constraint === Layout.Constraint.FIXED_COL && this.keysContainer.startAxis === Layout.AxisDirection.HORIZONTAL) {
            ...
            var numCol = this.keysContainer.constraintNum;
          ...
          if (this.keysContainer.constraint === Layout.Constraint.FIXED_ROW && this.keysContainer.startAxis === Layout.AxisDirection.VERTICAL) {
            // Only update height.
            var numRow = this.keysContainer.constraintNum;
        ...
        _updatePopupPosition() {
          ...
          var editBoxWorldBounds = this._editBoxUITransform.getBoundingBoxToWorld(); // Resize the keypad based on the layout.
          // Update the layout and background.
          // Get the width needed.


          var popupSize = this._uiTransform.contentSize = this._calculatePopupSize();

          this._keysContainerUI.contentSize = popupSize;

          var canvasBounds = this._parentCanvas.node._uiProps.uiTransformComp.getBoundingBox(); // Initially set anchor to bottom center.


          var anchorX = 0.5;
          var anchorY = 0.0; // Set the position Y as above the edit box bounds.

          var arrowRot = 180;
          var popupWorldPosY = editBoxWorldBounds.yMax + this._arrowSize.height; // Check if popupPositionY provides enough room for the entire popup to
          ...
          var popupWorldPosX = editBoxWorldBounds.center.x; // Check if enough space to the right of the popup. If not move the
          // popup to the left.

          if (popupWorldPosX + 0.5 * popupSize.width > canvasBounds.xMax - SCREEN_PADDING_X) {
            // NOTE: Assuming editbox center is inside screen.
            var widthAvailable = canvasBounds.width - SCREEN_PADDING_X - popupWorldPosX;
          ...
          // If not move the popup to the right.
            ...
            var _widthAvailable = popupWorldPosX - SCREEN_PADDING_X;

            anchorX = _widthAvailable / popupSize.width;
          }
        ...
        _createKeys() {
          if (this._showEmpty) {
            ...
            var emptyNode = instantiate(this.emptyContentPrefab);
        ...
        _addSoftKey(keyData) {
          ...
          var softKey = keyNode.getComponent(_crd && SoftKey === void 0 ? (_reportPossibleCrUseOfSoftKey({
            error: Error()
          }), SoftKey) : SoftKey);
          ...
          var keyUITransform = keyNode.getComponent(UITransform);
...
//# sourceMappingURL=3d22a9415a48e5f6a08fcf9eb6fc0c5813bdffb1.js.map