    ...
    execute: function () {
      ...
      _export("UIElement", UIElement = (_dec = ccclass("UIElement"), _dec2 = requireComponent(UITransform), _dec3 = property({
        ...
        set heightUnit(value) {
          ...
          var parent = this._yogaNode.getParent();
        ...
        onDisable() {
          ...
          var parentElement = (_this$node$parent = this.node.parent) == null ? void 0 : _this$node$parent.getComponent(UIElement);
        ...
        _commitChildren() {
          var actualChildrenCount = this._yogaNode.getChildCount();

          var children = this.node.children.map(child => child.getComponent(UIElement)).filter(element => !!element && element.enabledInHierarchy && !element._hasCanvas && !element._isFloating);

          if (children.length === this._committedChildren.length && children.every((child, index) => child === this._committedChildren[index])) {
            return;
          }

          for (var i = 0; i < Math.max(children.length, this._committedChildren.length); i++) {
            var oldChild = this._committedChildren[i];
            var correctChild = children[i];
        ...
        _updateLayoutInternal() {
          ...
          var top = this._yogaNode.getComputedTop();

          var left = this._yogaNode.getComputedLeft();

          var height = (_this$_yogaNode$getCo = this._yogaNode.getComputedHeight()) != null ? _this$_yogaNode$getCo : 0;
          var width = (_this$_yogaNode$getCo2 = this._yogaNode.getComputedWidth()) != null ? _this$_yogaNode$getCo2 : 0;
          uiTransform.setContentSize(width, height);

          var yogaParent = this._yogaNode.getParent();
          ...
          if (yogaParent == null) {
            ...
            var parentUiTransform = (_this$node$parent3 = this.node.parent) == null ? void 0 : _this$node$parent3._uiProps.uiTransformComp;
            ...
            var parentLeft = -parentUiTransform.width * parentUiTransform.anchorX;
            var parentTop = parentUiTransform.height * (1 - parentUiTransform.anchorY); // Get the top left corner of current node using the parent's top left and the calculated
            // top and left values from Yoga.

            var localLeft = parentLeft + left;
            var localTop = parentTop - top; // +ve y is bottom to top.
...
//# sourceMappingURL=bdb9732edcdfcf20ad11b2b0c9ba61dcc73a0034.js.map