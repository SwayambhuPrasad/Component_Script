      ...
      _export("UIDragSort", UIDragSort = (_dec = ccclass("UIDragSort"), _dec2 = property(Layout), _dec3 = property({
        ...
        updateShadow(drag) {
          ...
          if (this._shadowMap.has(drag)) {
            ...
            var shadowUI = shadow.addComponent(UITransform);
            var dragUI = drag.getComponent(UITransform);
        ...
        __addDrag(drag) {
          ...
          var shadow = this.updateShadow(drag);
        ...
        __removeDrag(drag) {
          ...
          var shadow = this._shadowMap.get(drag);
        ...
        _addEventListeners() {
          for (var child of this.node.children) {
            var drag = child.getComponent(_crd && UIDrag === void 0 ? (_reportPossibleCrUseOfUIDrag({
              error: Error()
            }), UIDrag) : UIDrag);
        ...
        _onRegisteredDragUpdate(drag) {
          ...
          var currentShadow = this._shadowMap.get(drag);

          var dragUITransform = drag.getComponent(UITransform);
          var bounds = dragUITransform.getBoundingBoxToWorld();

          for (var [otherDrag, shadow] of this._shadowMap.entries()) {
            if (otherDrag && otherDrag !== drag) {
              var shadowUITransform = shadow.getComponent(UITransform);
              var shadowBounds = shadowUITransform.getBoundingBoxToWorld();
          ...
          for (var [d, _shadow] of this._shadowMap.entries()) {
            if (!d.isDragging) d.targetWorldPosition = _shadow.worldPosition;
          }
        }
...
//# sourceMappingURL=7a2093534170f626057412318ec686d1c13c017c.js.map