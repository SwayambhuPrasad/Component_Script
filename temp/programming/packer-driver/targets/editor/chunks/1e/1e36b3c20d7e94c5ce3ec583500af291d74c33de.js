    ...
    execute: function () {
      ...
      _export("LatexRenderer", LatexRenderer = (_dec = ccclass("LatexRenderer"), _dec2 = requireComponent(UITransform), _dec3 = property({
        ...
        _render() {
          ...
          const nodeBuilder = new (_crd && VirtualNodeBuilder === void 0 ? (_reportPossibleCrUseOfVirtualNodeBuilder({
            error: Error()
          ...
          const rootY = nodeData.rootNode.strutBounds.y; // The position offset used to adjust position based on anchor.
          ...
          const baseUiXform = (_this$_baseNode$getCo = this._baseNode.getComponent(UITransform)) != null ? _this$_baseNode$getCo : this._baseNode.addComponent(UITransform);
        ...
        _updateOverflow() {
          ...
          switch (this._overflow) {
            ...
            case Overflow.CLAMP:
              ...
              const mask = (_this$getComponent = this.getComponent(Mask)) != null ? _this$getComponent : this.addComponent(Mask);
        ...
        _updateInputNodes() {
          ...
          const baseUI = (_this$_baseNode = this._baseNode) == null ? void 0 : _this$_baseNode._uiProps.uiTransformComp;
          ...
          this._inputClassData.forEach(({
            ...
            const worldPos = baseUI.convertToWorldSpaceAR(v3(x, -y)); // Offset the position based on anchor point of the input ui.
        ...
        _virtualLineNodeToCocos(virtualNode, baseNode) {
          ...
          const node = (_gfxNodePool$get = gfxNodePool.get()) != null ? _gfxNodePool$get : new Node("LatexGraphics");
          ...
          const uiXform = (_node$getComponent = node.getComponent(UITransform)) != null ? _node$getComponent : node.addComponent(UITransform);
          uiXform.setAnchorPoint(0, 0);
          uiXform.setContentSize(width, height);
          const gfx = (_node$getComponent2 = node.getComponent(Graphics)) != null ? _node$getComponent2 : node.addComponent(Graphics);
        ...
        _virtualBoxNodeToCocos(virtualNode, baseNode) {
          ...
          const node = (_gfxNodePool$get2 = gfxNodePool.get()) != null ? _gfxNodePool$get2 : new Node("LatexGraphics");
          node.hideFlags |= CCObject.Flags.DontSave | CCObject.Flags.HideInHierarchy;
          node.layer = this.node.layer;
          baseNode.addChild(node);
          const {
          ...
          const uiXform = (_node$getComponent3 = node.getComponent(UITransform)) != null ? _node$getComponent3 : node.addComponent(UITransform);
          uiXform.setAnchorPoint(0, 0);
          uiXform.setContentSize(width, height);
          const gfx = (_node$getComponent4 = node.getComponent(Graphics)) != null ? _node$getComponent4 : node.addComponent(Graphics);
          gfx.lineCap = Graphics.LineCap.BUTT;
          gfx.lineJoin = Graphics.LineJoin.MITER;
          gfx.miterLimit = 4;
        ...
        _virtualTextNodeToCocos(virtualNode, baseNode) {
          ...
          const node = (_labelNodePool$get = labelNodePool.get()) != null ? _labelNodePool$get : new Node("LatexText");
          node.hideFlags |= CCObject.Flags.DontSave | CCObject.Flags.HideInHierarchy;
          node.layer = this.node.layer;
          baseNode.addChild(node);
          const uiXform = (_node$getComponent5 = node.getComponent(UITransform)) != null ? _node$getComponent5 : node.addComponent(UITransform);
          const {
            x,
            y,
            width,
          ...
          const fontData = virtualNode.font.split(" "); // px to pt conversion 1px = 0.75pts

          const fontSize = +fontData[2].replace(/px/gi, "") * 0.75; // The cocos creator label is a bit wierd with base line height. Hence this adjustment.

          uiXform.setAnchorPoint(0, 0.18);
          uiXform.setContentSize(width, height);
          const label = (_node$getComponent6 = node.getComponent(Label)) != null ? _node$getComponent6 : node.addComponent(Label);
        ...
        _virtualSvgNodeToCocos(virtualNode, baseNode) {
          ...
          node.layer = this.node.layer;
          baseNode.addChild(node);
          const uiXform = (_node$getComponent7 = node.getComponent(UITransform)) != null ? _node$getComponent7 : node.addComponent(UITransform);
          uiXform.setAnchorPoint(0, 0);
          const sprite = (_node$getComponent8 = node.getComponent(Sprite)) != null ? _node$getComponent8 : node.addComponent(Sprite);
          const {
            x,
            y
          } = virtualNode.bounds;
          node.position = v3(x, -y);
          const img = new Image();
          const svg = new Blob([virtualNode.virtualHtmlNode.toMarkup()], {
            type: "image/svg+xml"
          });
...
//# sourceMappingURL=1e36b3c20d7e94c5ce3ec583500af291d74c33de.js.map