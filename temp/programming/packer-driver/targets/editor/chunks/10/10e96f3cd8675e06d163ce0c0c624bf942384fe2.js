    ...
    execute: function () {
      ...
      _export("StoryPanel", StoryPanel = (_dec = ccclass("StoryPanel"), _dec2 = property({
        ...
        _DragEnd(drag, Index) {
          ...
          this._stateManager[Index] = false;

          if (slot.replacedNode.length != 0) {
            slot.replacedNode[0].node.active = true;

            this._dragNodes.forEach((d, i) => {
              if (d === slot.replacedNode[0].node) this._stateManager[i] = true;
            });
          }

          console.log(this._stateManager); //drag.node.getComponent(UIDrag).reset();
        ...
        _pageNavigation(pageNumber) {
          ...
          for (let i = 0; i < pageGrouping.length; i++) for (let j = 0; j < pageGrouping[i].length; j++) {
            if (i == pageNumber) {
              pageGrouping[i][j].active = true; //this._stateManager[i][j].push(true);
            } else {
              pageGrouping[i][j].active = false; //this._stateManager[i][j].push(false);
            }
          }
...
//# sourceMappingURL=10e96f3cd8675e06d163ce0c0c624bf942384fe2.js.map