...
interface PapaParse {
  ...
  parse<T, TFile extends LocalFile = LocalFile>(
    file: TFile,
   ...
   */
  // tslint:disable-next-line: no-unnecessary-generics
  parse<T>(url: string, config: ParseRemoteConfig<T>): void;
  /**
  ...
  BAD_DELIMITERS: ReadonlyArray<string>;
   ...
   */
  readonly NODE_STREAM_INPUT: unique symbol;
   ...
   */
  Parser: new (config: ParseConfig) => Parser;
}

/** File object */
export type LocalFile = Blob;
